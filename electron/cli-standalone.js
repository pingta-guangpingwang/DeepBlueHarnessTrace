    ...
    try {
        ...
        for (const entry of registry) {
            for (const wc of entry.workingCopies) {
                ...
                let lastUpdate = '';
                try {
                    lastUpdate = (await fs.stat(wc.path)).mtime.toISOString();
                }
                catch { /* ok */ }
                projects.push({
                    name: entry.name,
    ...
    try {
        ...
        if (await fs.pathExists(linkPath)) {
            try {
                ...
                const registry = await readProjectRegistry(rootPath);
                const entry = registry.find((e) => path.resolve(e.repoPath) === path.resolve(link.repoPath));
...
async function resolveRepoPaths(inputPath) {
...
program.command('commit <projectPath>')
    ...
    try {
        ...
        let files = opts.files ? opts.files.split(',').map((f) => f.trim()) : [];
        if (!files.length) {
            const s = await repo.getStatus(repoPath, workingCopyPath);
            if (s.success && s.status) {
                files = s.status.filter((l) => l.startsWith('A ') || l.startsWith('M ') || l.startsWith('D ')).map((l) => l.slice(2).trim());
            }
        }
        if (!files.length) {
        ...
        const options = {};
        if (opts.ai && opts.ai !== 'manual')
            options.author = opts.ai;
        if (opts.session)
        ...
        const tick = async () => {
            try {
                ...
                if (!statusResult.success)
                    return;
                const changedFiles = (statusResult.status || []).filter((s) => s.startsWith('[新增]') || s.startsWith('[修改]') || s.startsWith('[删除]'));
                ...
                if (allStatus.success && allStatus.status) {
                    // 从状态行提取文件路径（取所有状态）
                    filesToCommit = allStatus.status.map((l) => {
                        const idx = l.indexOf('] ');
...
program.command('pull <repoPath> <targetDir>')
    ...
    .action(async (repoPath, targetDir, opts) => {
    const fmt = program.opts().format;
    try {
...
program.command('git-connect <projectPath> <remoteUrl>')
    ...
    .action(async (projectPath, remoteUrl, opts) => {
    const fmt = program.opts().format;
    try {
        projectPath = path.resolve(projectPath);
        ...
        if (!(await fs.pathExists(gitDir))) {
            await isomorphic_git_1.default.init({ fs, dir: projectPath, defaultBranch: branch });
        }
        // Set remote
        const remotes = await isomorphic_git_1.default.listRemotes({ fs, dir: projectPath });
        if (remotes.find((r) => r.remote === 'origin')) {
            await isomorphic_git_1.default.deleteRemote({ fs, dir: projectPath, remote: 'origin' });
        }
        await isomorphic_git_1.default.addRemote({ fs, dir: projectPath, remote: 'origin', url: remoteUrl });
        // Fetch + checkout
        try {
            await isomorphic_git_1.default.fetch({ fs, http: node_1.default, dir: projectPath, remote: 'origin', ref: branch, onAuth, depth: 1 });
            try {
                await isomorphic_git_1.default.checkout({ fs, dir: projectPath, ref: branch, force: true });
            }
            catch {
                await isomorphic_git_1.default.branch({ fs, dir: projectPath, ref: branch, checkout: true });
                await isomorphic_git_1.default.checkout({ fs, dir: projectPath, ref: branch, force: true });
            }
        }
        catch (fetchError) {
...
program.command('git-pull <projectPath>')
    ...
    try {
        ...
        let branch = 'main';
        try {
            branch = (await isomorphic_git_1.default.currentBranch({ fs, dir: projectPath })) || 'main';
        }
        catch { /* use default */ }
...
program.command('git-push <projectPath>')
    ...
    try {
        ...
        // Stage all
        const matrix = await isomorphic_git_1.default.statusMatrix({ fs, dir: projectPath });
        for (const [filepath, headStatus, workdirStatus] of matrix) {
            if (headStatus !== workdirStatus) {
                await isomorphic_git_1.default.add({ fs, dir: projectPath, filepath: filepath });
            }
        }
        // Commit if needed
        const stagedMatrix = await isomorphic_git_1.default.statusMatrix({ fs, dir: projectPath });
        ...
        // Push
        await isomorphic_git_1.default.push({ fs, http: node_1.default, dir: projectPath, remote: 'origin', ref: branch, onAuth });
...
program.command('unregister <projectPath>')
    ...
    try {
        ...
        for (let i = registry.length - 1; i >= 0; i--) {
            ...
            const hadWC = entry.workingCopies.some((wc) => path.resolve(wc.path) === normalized);
            if (hadWC)
                found = true;
...
vectorCmd.command('ingest <projectName> [files...]')
    ...
    .action(async (projectName, files) => {
    const fmt = program.opts().format;
    try {
        const rootPath = getRootPath(program.opts());
        const registry = await readProjectRegistry(rootPath);
        ...
        const ingestResult = await (0, vector_engine_1.ingestFiles)(rootPath, resolvedFiles, projectName, commitId);
        if (fmt === 'table') {
...
vectorCmd.command('files <projectName>')
    ...
    try {
        const rootPath = getRootPath(program.opts());
        const result = await (0, vector_engine_1.getIndexedFiles)(rootPath, projectName);
        if (fmt === 'table') {
...
vectorCmd.command('export <projectName>')
    ...
    try {
        const rootPath = getRootPath(program.opts());
        const result = await (0, vector_engine_1.exportVectorIndex)(rootPath, projectName);
...
vectorCmd.command('import <projectName> <file>')
    ...
    try {
        ...
        const result = await (0, vector_engine_1.importVectorIndex)(rootPath, projectName, data);
        if (fmt === 'table') {
...
vectorCmd.command('remove <projectName> [files...]')
    ...
    try {
        const rootPath = getRootPath(program.opts());
        const result = await (0, vector_engine_1.removeFilesFromIndex)(rootPath, '', '', projectName, files);
        if (fmt === 'table') {
...
vectorCmd.command('supported')
    ...
    .action(() => {
    const result = (0, vector_engine_1.getSupportedExtensions)();
    console.log('\n📋 支持的导入文件格式:');
    console.log('═'.repeat(50));
    const byCategory = {};
    for (const ext of result.extensions) {
        if (!byCategory[ext.category])
