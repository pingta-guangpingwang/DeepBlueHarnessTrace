...
function registerProjectHandlers(ipcMain, mainWindow, dbvsRepo) {
    // 创建新项目：在 repositories/<name> 创建集中仓库，创建工作副本
    ipcMain.handle('dbgvs:create-project', async (_, rootPath, projectName, customPath) => {
        try {
            if (!projectName?.trim()) {
    ...
    ipcMain.handle('dbgvs:get-projects', async (_, rootPath) => {
        try {
            const registry = await (0, project_registry_1.readProjectRegistry)(rootPath);
            const projectList = [];
    ...
    ipcMain.handle('dbgvs:register-project', async (_, rootPath, projectPath, projectName, initWithCommit = false, options) => {
        try {
            ...
            await (0, project_registry_1.writeProjectRegistry)(rootPath, registry);
            await (0, project_registry_1.removeExcludedRepo)(rootPath, repoPath);
    ...
    ipcMain.handle('dbgvs:checkout-project', async (_, rootPath, repoPath) => {
        try {
    ...
    // Checkout 到指定位置
    ipcMain.handle('dbgvs:checkout-to', async (_, rootPath, repoPath, targetParentDir, folderName) => {
        try {
    ...
    // 注册已有工作副本
    ipcMain.handle('dbgvs:register-working-copy', async (_, rootPath, workingCopyPath) => {
        try {
    ...
    // 从项目列表移除工作副本
    ipcMain.handle('dbgvs:unregister-project', async (_, rootPath, workingCopyPath) => {
        try {
            const registry = await (0, project_registry_1.readProjectRegistry)(rootPath);
            ...
            await (0, project_registry_1.writeProjectRegistry)(rootPath, registry);
            if (removedRepoPath) {
                await (0, project_registry_1.addExcludedRepo)(rootPath, removedRepoPath);
            }
    ...
    // 删除工作副本
    ipcMain.handle('dbgvs:delete-working-copy', async (_, rootPath, workingCopyPath) => {
        try {
            const normalized = path.resolve(workingCopyPath);
            ...
            for (let i = registry.length - 1; i >= 0; i--) {
                ...
                entry.workingCopies = entry.workingCopies.filter(wc => path.resolve(wc.path) !== normalized);
                if (entry.workingCopies.length === 0) {
                    await (0, project_registry_1.addExcludedRepo)(rootPath, entry.repoPath);
                    registry.splice(i, 1);
                }
            }
            await (0, project_registry_1.writeProjectRegistry)(rootPath, registry);
    ...
    // 更新项目排序
    ipcMain.handle('dbgvs:reorder-projects', async (_, rootPath, orderedRepos) => {
        try {
            const registry = await (0, project_registry_1.readProjectRegistry)(rootPath);
    ...
    // 设置项目重要程度
    ipcMain.handle('dbgvs:set-project-rating', async (_, rootPath, repoPath, rating) => {
        try {
    ...
    // 删除仓库
    ipcMain.handle('dbgvs:delete-repository-full', async (_, rootPath, repoPath, deleteWorkingCopies) => {
        try {
            const registry = await (0, project_registry_1.readProjectRegistry)(rootPath);
    ...
    ipcMain.handle('dbgvs:ensure-project-docs', async (_, rootPath) => {
        try {
            const registry = await (0, project_registry_1.readProjectRegistry)(rootPath);
            let updated = 0;
            for (const entry of registry) {
                for (const wc of entry.workingCopies) {
                    if (await fs.pathExists(wc.path)) {
                        ...
                        if (!beforeExists)
                            ...
                            if (!content.includes('<!-- DBHT-GUIDE-VERSION: 7 -->'))
                                updated++;
                        }
                    }
                }
    ...
    // 创建根仓库
    ipcMain.handle('dbgvs:create-root-repository', async (_, rootPath) => {
        try {
    ...
    // 保存根仓库配置
    ipcMain.handle('dbgvs:save-root-repository', async (_, rootPath) => {
        try {
    ...
    ipcMain.handle('dbgvs:set-onboarding-completed', async (_, completed) => {
        try {
            const onboardingPath = path.join(electron_1.app.getPath('userData'), 'onboarding.json');
...
var project_registry_2 = require("../project-registry");
Object.defineProperty(exports, "getProjectsList", { enumerable: true, get: function () { return project_registry_2.getProjectsList; } });
