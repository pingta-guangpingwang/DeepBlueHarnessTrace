...
### 📋 项目排序与评分
- **手动排序**：每个项目卡片上的 ⏫ ▲ ▼ ⏬ 按钮，精确控制排列位置
- **星级评分 1–6 级**：绿色(1–2)、黄色(3–4)、红色(5–6)，点击弹出颜色编码下拉选择器
- **排序切换**：手动排序 / 重要程度排序一键切换，偏好自动保存
- **拖拽排序**：按住任意项目卡片即可拖拽重排，拖到列表边缘自动滚动
- 排序和评分数据通过 IPC 持久化到项目注册表，重启不丢失
